...
for msg in iter(lambda: comm.sendrecv(None, dest=0), None):
    if msg!=StopIteration:
        ...
        if mesh_generator=="gmsh":
            ...
            local_formation_geometry, local_borehole_geometry, sigma = gmf.SelectGmshDataRange(borehole_geometry, formation_parameters, dip, mud_resistivities[depth_index], simulation_depths[depth_index])
    ...
    if len(tasks)==computation_batch_size or msg==StopIteration:
        ...
        for i in range(len(tasks)):
            task = tasks[i]
            local_formation_geometry = local_formation_geometry_list[i]
            local_borehole_geometry = local_borehole_geometry_list[i]
            tool_geometry = tool_geometry_list[i]
