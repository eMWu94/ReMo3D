...
for task in iter(lambda: comm.sendrecv(None, dest=0), StopIteration):
    # try:
    simulation_depth = task[0]
    simulated_tool = task[1]
    tool_geometry = simulated_tool[0,:]
    source_terms = simulated_tool[1,:]
    if mesh_generator=="gmsh":
        # Carve out suitable range of data
        local_borehole_geometry = rm.SelectGmshBoreholeDataRange(borehole_geometry, dip, simulation_depths[simulation_depth], domain_radius)
        local_formation_geometry, local_formation_resistivity = rm.SelectGmshFormationDataRange(formation_parameters, dip, simulation_depths[simulation_depth], domain_radius)
        ...
        sigma = CoefficientFunction([1/mud_resistivities[simulation_depth]] + list(1/local_formation_resistivity)) # Conductivity distribution within the model
        dirichlet_boundary = 'dirichlet_boundary'
    elif mesh_generator=="netgen":
        # Carve out suitable range of data
        local_formation_geometry, local_borehole_geometry, sigma = rm.SelectNetgenDataRange(borehole_geometry, formation_parameters, mud_resistivities[simulation_depth], simulation_depths[simulation_depth], domain_radius)
    ...
    resistivity_computation_tasks = task[2]
